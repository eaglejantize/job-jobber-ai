...
function readDetailError(detail: Json | null): string | null {
  if (!detail || typeof detail !== "object" || Array.isArray(detail)) return null;

  const obj = detail as Record<string, Json | undefined>;
  const preferred = obj.error ?? obj.reason;
    ...
    >
      ...
      <DialogContent>
          ...
          {result?.callId && (
            <div className="rounded-md border border-border bg-background p-3 text-xs space-y-1.5">
              ...
              {STEP_LABELS.map(({ step, label }) => {
                ...
                const errorText =
                  ev?.status === "error" ? readDetailError(ev.detail) : null;
            ...
            <div className="rounded-md border border-border bg-secondary/30 p-3 text-xs space-y-1">
              ...
              {showDetails && (
                <div className="space-y-1 pt-2">
                  ...
                  {result.callId && (
                    <div>
                      <span className="text-muted-foreground">Call ID:</span> {result.callId}
                    </div>
                  )}
        ...
        <DialogFooter>
          ...
          <Button onClick={() => void placeCall()} disabled={status === "calling" || status === "connected"}>
            {status === "calling" ? (
              <>
