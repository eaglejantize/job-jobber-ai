...
import { DEMO_NUMBER, DEMO_NUMBER_AVAILABLE } from "@/lib/constants";
  ...
  return (
    <div className="rounded-2xl border border-primary/30 bg-card/80 backdrop-blur p-6 md:p-8 shadow-card-soft">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <a
            href={DEMO_NUMBER_AVAILABLE ? `tel:${DEMO_NUMBER.replace(/\D/g, "")}` : undefined}
            className="block text-2xl md:text-3xl font-bold tracking-wider tabular-nums"
          >
            {DEMO_NUMBER}
          </a>
          <p className="text-xs text-muted-foreground mt-0.5">
            {DEMO_NUMBER_AVAILABLE ? "Tap to call" : "Demo number coming soon"}
          </p>
        </div>
      </div>
    </div>
