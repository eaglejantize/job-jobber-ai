...
Deno.serve(async (req) => {
  ...
  const updateLead = async (patch: Json) => {
    if (!lead_id) return;
    await supabase.from("callcapture_leads").update(patch).eq("id", lead_id);
  };
  ...
  const raw = (leadRow.raw_payload ?? {}) as Json;
  const intake = (leadRow.intake_answers ?? {}) as Json;
  ...
  if (callVapiId) {
    ...
    recordingUrl = (callRow as { recording_url?: string | null } | null)?.recording_url ?? null;
    callSummary = (callRow as { issue_summary?: string | null } | null)?.issue_summary ?? null;
  }

  const { day, time } = parseTiming(leadRow.timing as string | null);
  ...
  const serviceLabel =
    ...
    (client as { industry?: string | null } | null)?.industry ??
    "service request";
  const issueText =
    leadRow.summary ??
  ...
  const summaryParts = [
    ...
    leadRow.timing ? `timing=${leadRow.timing}` : null,
  ].filter(Boolean);
  ...
  const body = {
    ...
    customer_name: leadRow.name ?? null,
